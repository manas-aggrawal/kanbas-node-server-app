...
export default function QuizSubmissionRoute(app) {
        ...
        if (question.type === "mcq") {
          ...
          if (
            question.possibleAnswers
              ...
              .includes(response.answer.toString().toLowerCase())
          ) {
            score += question.points;
          }
        }
  ...
  app.post("/api/quizzes/all-attempts-for-course", async (req, res) => {
    ...
    const userId = req.body.userId;
    const fetchedAttempts = await quizSubmissionDao.findAllQuizAttemptForCourse(
      courseId,
      userId
    );
    ...
    const latestAttempts = fetchedAttempts.reduce(
      (accumulator, currentAttempt) => {
        ...
        if (
          !accumulator[currentAttempt.quiz] ||
          new Date(accumulator[currentAttempt.quiz].submittedAt) <
